...
from dajaxexamples.examples import settings as game_settings
...
commands = None
try:
    sys.path.append(game_settings.GAME_FILES)
    base = re.findall(r'^(\w+)\.py$', game_settings.CMD_CODE)
    if len(base) == 0:
        raise NameError
    game_commands = __import__(base[0])
except:
    raise NameError("Need to specify a file for setting CMD_CODE!")
...
def multiply(request, a, b):
    dajax = Dajax()
    result = getattr(game_commands, "multiply")(int(a), int(b))
...
def run_cmd(request, a, b):
    dajax = Dajax()
    result = int(a) * int(b)
    dajax.assign('#result','value',str(result))
    return dajax.json()

dajaxice_functions.register(run_cmd)
